...
t_token	*find_next_ast_node_with_pipe(t_token *last)
{
	...
	while (last && last->id != PIPE && !is_operator(last->text[0]))
	{
		tmp = last;
		last = last->prev;
	}
	if (last->id == PIPE || !last)
		return (tmp);
	return (last);
}
...
bool	fill_left(t_ast **ast, t_token *last)
{
	...
	while (nb_operators)
	{
		...
		if ((*ast)->left->parent == NULL)
			return (false);
		(*ast)->left->parent = *ast;
		if ((*ast)->left->id == CMD && last->next->id == ARGUMENT)
		{
...
bool	fill_left_inside(t_ast **ast, t_token *token)
{
	...
	while (nb_operators_inside_pipe)
	{
		...
		if ((*ast)->left->parent == NULL)
			return (false);
		(*ast)->left->parent = *ast;
		if (token->id == CMD && token->next->id == ARGUMENT)
		{
